...
# Atualizar a stack
aws cloudformation update-stack --stack-name MatStack --template-body file://projeto_aws.yaml --capabilities CAPABILITY_IAM

# Verificar se o comando anterior foi bem-sucedido
if [ $? -eq 0 ]; then
    echo "Aguardando atualização..."
    # Aguardar até a atualização estar completa
    aws cloudformation wait stack-update-complete --stack-name MatStack
